...
// Crear un QoS adecuado para el Subscriber
rmw_qos_profile_t qos_profile = rclcpp::SensorDataQoS().get_rmw_qos_profile();

// Crear un nodo compartido
auto lifecycle_node = std::make_shared<rclcpp_lifecycle::LifecycleNode>("node_name");

// Convertir el LifecycleNode a un rclcpp::Node
auto node = std::dynamic_pointer_cast<rclcpp::Node>(lifecycle_node);

// Crear el Subscriber con el QoS correcto
message_filters::Subscriber<sensor_msgs::msg::LaserScan> subscriber(
    node, "topic_name", qos_profile);
